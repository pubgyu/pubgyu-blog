  ...
  var p5VoiceWave = function p5VoiceWave(p) {
    ...
    p.draw = function () {
      if (document.querySelector('.section-voice').classList.contains('show')) {
        ...
        for (var i = 0; i < sinarioMotion.voiceSinario.wave.length; i++) {
          drawCircle(sinarioMotion.voiceSinario.wave[i].scroll, sinarioMotion.voiceSinario.wave[i].stroke);
        }
      }
    };
    ...
    function drawCircle(percent, stroke) {
      var size = prop.minSize + prop.maxSize * percent / 2; // const strokeThin = 20 * Math.sin(Math.PI * Math.min(1, percent * 3));
      // const strokeThick = 1 * easeOutExpo(percent);

      p.push();
      p.stroke(stroke);
      p.strokeWeight(prop.strokeWidth);
      STORY.polygon(p, prop.center.x, prop.center.y, size / 2 - prop.strokeWidth / 2, 100); // STORY.polygon(p, prop.center.x, prop.center.y, size / 2 - strokeThick / 2, 100);
      // p.stroke(thin);
  ...
  var p5DotMotion = function p5DotMotion(p) {
    ...
    p.draw = function () {
      if (canvasDotMotion.closest('.sticky-wrap').classList.contains('show')) {
        (function () {
          p.clear();
          p.translate(STORY.wW / 2, STORY.wH / 2); // tail
          ...
          smallWaveDraw(p, 0, _row, 0, _row, stss.circlaTail3); // smallWaveDraw(p, _colum, _row * 2, _colum, _row * 2, stss.circlaTail4);

          smallWaveDraw(p, 0, -_row, 0, -_row, stss.circlaTail5);
          smallWaveDraw(p, -_colum, 0, -_colum, 0, stss.circlaTail6); // big wave

          for (var _i = 0; _i < bigWaves.length; _i++) {
            bigWaveDraw(p, bigWaves[_i]);
          }
  ...
  var p5SphereMotion = function p5SphereMotion(p) {
    ...
    p.draw = function () {
      if (canvasSphereMotion.closest('.sticky-wrap').classList.contains('show') && !canvasDottedMotion.closest('.sticky-wrap').classList.contains('show')) {
        p.clear();
        p.background(sps.bgFill.fill); // small ball ground
        ...
        p.rotateY(_camera.rotateY + autoRotateY); // center ball

        centerSphere(p); // trackSphere
        ...
        for (var _i5 = 0; _i5 < waves.length; _i5++) {
          waveDraw(p, waves[_i5]);
        }

        p.camera(_camera.x, _camera.y, _camera.z); // dimmed
